...
glraytracer::glraytracer(int w, int h) {
	...
	reader = NULL;
        ...
        PRP = new point3D(0,0,0);
        VRP = new point3D(0,0,-1);
        VPN = new point3D(0,0,1);
        VUP = new point3D(0,1,0);
...
void glraytracer::keyboard(unsigned char key, int x, int y) {
    if (key == 'q' || key == 'Q')
        exit(0);
}

// special keyboard function - called when user has special keys input
void glraytracer::specialKey(int key, int x, int y) {
	
}

// mouse button function - called when user clicks any mouse button
void glraytracer::mouseButton(int button, int state, int x, int y) {
	
}

// idle function - called when no other function is running
void glraytracer::idleFunc() {
